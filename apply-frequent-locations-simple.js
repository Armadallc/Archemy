...
async function applyMigration() {
  ...
  try {
    ...
    console.log('🔍 Checking current frequent_locations schema...');
    ...
    console.log('✅ Current schema checked');

    // Step 2: Update existing frequent locations to have 'other' tag
    ...
    console.log(`📊 Found ${existingLocations.length} existing frequent locations`);

    // Update each location with default values
    ...
    // Step 3: Sync existing service locations
    ...
    for (const location of locations) {
      ...
      if (existing) {
        console.log(`⏭️ Location ${location.name} already synced, skipping`);
        continue;
      }
