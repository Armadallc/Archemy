...
export default function PermissionsPage() {
  ...
  const availableResources = ['*', 'program', 'corporate_client', 'own'];
  ...
  const getPermissionIcon = (permission: string) => {
    if (permission.includes('TRIPS')) return <Flag className="w-4 h-4" />;
    if (permission.includes('DRIVERS')) return <Users className="w-4 h-4" />;
    if (permission.includes('CLIENTS')) return <Users className="w-4 h-4" />;
    if (permission.includes('VEHICLES')) return <Settings className="w-4 h-4" />;
    if (permission.includes('USERS')) return <Users className="w-4 h-4" />;
    if (permission.includes('PERMISSIONS')) return <Shield className="w-4 h-4" />;
    if (permission.includes('ANALYTICS')) return <Settings className="w-4 h-4" />;
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">PERMISSIONS & FEATURE FLAGS</h1>
