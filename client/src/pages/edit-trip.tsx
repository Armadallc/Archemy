...
export default function EditTrip() {
  ...
  const { data: clients = [] } = useQuery({
    ...
    queryFn: async () => {
      const endpoint = selectedProgram ? `/api/clients/program/${selectedProgram}` : "/api/clients";
  ...
  const { data: clientGroups = [] } = useQuery({
    queryKey: ["/api/client-groups", selectedProgram],
    queryFn: async () => {
      const endpoint = selectedProgram ? `/api/client-groups/program/${selectedProgram}` : "/api/client-groups";
      const response = await apiRequest("GET", endpoint);
      return await response.json();
    },
    enabled: !!selectedProgram,
  ...
  const { data: drivers = [] } = useQuery({
    ...
    queryFn: async () => {
      const endpoint = selectedProgram ? `/api/drivers/program/${selectedProgram}` : "/api/drivers";
      const response = await apiRequest("GET", endpoint);
      return await response.json();
    },
    enabled: !!selectedProgram,
