...
export function TripHoverCard({ trip, children }: TripHoverCardProps) {
  ...
  const handleEditTrip = () => {
    ...
    // Extract base path to check for calendar/trips pages
    const basePath = currentPath.replace(/^\/corporate-client\/[^/]+/, '').replace(/^\/corporate-client\/[^/]+\/program\/[^/]+/, '');
    
    // Check if we're on calendar page (including hierarchical URLs)
    if (basePath === '/calendar' || basePath.startsWith('/calendar') || currentPath.includes('/calendar')) {
      sessionStorage.setItem('previousPath', currentPath);
    } else {
      // Store the current path if it's valid (allow hierarchical URLs)
      if (currentPath && 
          basePath !== `/trips/edit/${trip.id}` && 
          currentPath !== `/trips/edit/${trip.id}` &&
          basePath !== '/trips/new' && 
