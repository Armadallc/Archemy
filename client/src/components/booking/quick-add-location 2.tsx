...
interface QuickAddLocationProps {
  ...
  placeholder?: string;
  locationType?: 'pickup' | 'dropoff' | 'destination' | 'facility' | 'courthouse' | 'medical' | 'commercial' | 'other';
  label?: string;
  required?: boolean;
}
...
export default function QuickAddLocation({
  ...
  const { data: frequentLocations = [], isLoading } = useQuery({
    ...
    enabled: true, // Always enable, let the API handle filtering
  });
              ...
              <CommandList>
                ...
                <CommandGroup>
                  {filteredLocations.map((location) => {
                    const IconComponent = locationTypeIcons[location.location_type];
                    return (
                      <CommandItem
                        key={location.id}
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          <Badge className={locationTypeColors[location.location_type]} size="sm">
...
function CreateLocationDialog({ 
  ...
  const [formData, setFormData] = useState({
    ...
    state: '',
    zip_code: '',
    location_type: (defaultLocationType || 'destination') as const,
  });
