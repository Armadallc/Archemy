...
export default function EnhancedActivityFeed() {
  ...
  const { refreshNow } = useRealTimeUpdates({
    ...
    queryKeys: [
      `["/api/trips","${level}","${selectedProgram}","${selectedCorporateClient}"]`,
      `["/api/clients","${level}","${selectedProgram}","${selectedCorporateClient}"]`,
      `["/api/client-groups","${level}","${selectedProgram}","${selectedCorporateClient}"]`
    ]
  });
    ...
    const generateActivities = () => {
      ...
      trips.slice(0, 8).forEach((trip: any) => {
        ...
        if (trip.status === 'completed') {
          ...
          activityItems.push({
            ...
            description: `Successfully completed pickup for ${trip.client?.first_name || 'client'}`,
        ...
        if (trip.status === 'cancelled') {
          activityItems.push({
            ...
            description: `Trip cancelled - ${trip.client?.first_name || 'Client'}`,
      ...
      clients.slice(0, 3).forEach((client: any) => {
        activityItems.push({
          ...
          description: `${client.first_name} ${client.last_name} added to system`,
