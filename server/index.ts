...
app.options('*', (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'CORS preflight failed' });
  }
});
...
(async () => {
  ...
  app.get("/api/ws-test", (req, res) => {
    try {
      ...
      if (wsServer) {
        ...
        const clientDetails = Array.isArray(clients) ? clients.map(c => ({
          userId: c.userId || 'unknown',
          role: c.role || 'unknown',
          programId: c.programId || null,
          corporateClientId: c.corporateClientId || null,
          authorizedPrograms: c.authorizedPrograms || []
        })) : [];
      ...
      console.error('❌ Error in /api/ws-test:', error);
      res.json({
        status: "error",
  ...
  // NOTE: Removed fallback permissions endpoint - using real route in server/routes/permissions.ts
  // The real route at /api/permissions/all uses the database and is mounted via server/routes/index.ts

  // Direct user creation endpoint before router registration
  app.post("/api/users", requireSupabaseAuth, requirePermission(PERMISSIONS.MANAGE_USERS), async (req: SupabaseAuthenticatedRequest, res) => {
    try {
      ...
      // Corporate admins can only create users within their own corporate client
      if (currentUser.role === 'corporate_admin') {
        if (!currentUser.corporateClientId) {
          return res.status(403).json({ 
            message: "Corporate client ID required for corporate admin" 
          });
        }
        
        // Ensure the new user is assigned to the same corporate client
        // Corporate admins can create other corporate_admin users (co-owners)
        // or any role within their corporate client
        const requestedCorporateClientId = req.body.corporate_client_id || req.body.corporateClientId;
        if (requestedCorporateClientId && requestedCorporateClientId !== currentUser.corporateClientId) {
          return res.status(403).json({ 
            message: "You can only create users within your own corporate client" 
          });
        }
        
        // Auto-assign corporate_client_id if not provided
        req.body.corporate_client_id = currentUser.corporateClientId;
      }
  ...
  // But only if the route hasn't been handled (response not sent)
  app.use('/api/*', (req, res, next) => {
    // Only send 404 if response hasn't been sent (route wasn't matched)
    if (!res.headersSent) {
  ...
  // Static file serving - only for non-API routes
  // API routes are handled above, so this will only serve frontend files
  ...
  // Serve the app on port 8081 (or PORT env variable for 8082)
  // this serves both the API and the client.
  const port = process.env.PORT ? parseInt(process.env.PORT) : 8081;
  const host = process.env.HOST || '0.0.0.0'; // Listen on all interfaces for Railway
  server.listen(port, host, () => {
    console.log(`🚀 HALCYON NMT Server running on ${host}:${port}`);
  });
})();