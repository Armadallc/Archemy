...
export async function verifySupabaseToken(token: string) {
  try {
    ...
    console.log('🔍 [BACKEND AUTH] Supabase URL:', supabaseUrl);
    console.log('🔍 [BACKEND AUTH] Supabase Anon Key (first 20 chars):', supabaseAnonKey.substring(0, 20));
    ...
    if (error) {
      ...
      console.log('❌ [BACKEND AUTH] Full error:', JSON.stringify(error, null, 2));
      return null;
    }
    ...
    if (dbError) {
      ...
      console.log('❌ [BACKEND AUTH] Database error details:', dbError.details);
      return null;
    }
...
export async function requireSupabaseAuth(req: SupabaseAuthenticatedRequest, res: express.Response, next: express.NextFunction) {
  try {
    ...
    console.log('🔍 [BACKEND AUTH] Authorization header:', req.headers.authorization ? `${req.headers.authorization.substring(0, 30)}...` : 'MISSING');
    console.log('🔍 [BACKEND AUTH] All headers:', JSON.stringify(Object.keys(req.headers)));
    
    const token = extractToken(req);
    
    if (!token) {
      console.log('❌ [BACKEND AUTH] No token found in request');
      console.log('❌ [BACKEND AUTH] Authorization header value:', req.headers.authorization || 'undefined');
      return res.status(401).json({ message: 'Not authenticated' });
    }

    console.log('🔍 [BACKEND AUTH] Token extracted, length:', token.length, 'first 30 chars:', token.substring(0, 30) + '...');
